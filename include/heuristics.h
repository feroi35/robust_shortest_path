...
struct HeuristicMethod : public SolveMethod {
    HeuristicMethod(const Instance& inst, const double& precision_K_ = 1e-6, const int& max_iter_ = 2000, const float& max_duration_ = 20.0);
    ~HeuristicMethod(){};
    ...
    std::vector<IloInt> retrieve_feasible_sol_2(const Instance& inst, IloEnv& env, const int& verbose) const;
...
struct NodesInfo{
    ...
    NodesInfo(): index(-1), parent(-1), dist(0), robust_dist(0), dist_nodes(0), robust_dist_nodes(0), knapsack_dij(), knapsack_phi() {};
    NodesInfo(int index, int parent, float dist, float robust_dist, float dist_nodes, float robust_dist_nodes, std::vector<std::tuple<float,float>> knapsack_dij, std::vector<float> knapsack_phi): index(index), parent(parent), dist(dist), robust_dist(robust_dist), dist_nodes(dist_nodes), robust_dist_nodes(robust_dist_nodes), knapsack_dij(knapsack_dij), knapsack_phi(knapsack_phi) {};
