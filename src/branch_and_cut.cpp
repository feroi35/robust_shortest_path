...
ILOLAZYCONSTRAINTCALLBACK4(myCallBack,const IloBoolVarArray&, x,
        const IloBoolVarArray&, y, const IloNumVar&, z, const Instance&, inst) {
    IloEnv env = getEnv();
    ////////////////////////////////
...
void branch_and_cut_solve(IloEnv env, Instance& inst, const unsigned int& time_limit, const int& verbose) {
    ...
    cplex.use(myCallBack(env,x,y,z,inst));
    if (verbose < 2) cplex.setOut(env.getNullStream());
    cplex.exportModel("model_debug.lp");
