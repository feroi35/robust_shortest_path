...
void DualizedMethod::solve(IloEnv& env, Instance& inst, const unsigned int& time_limit, const int& verbose) {
    ...
    if (reduce_symmetry) {
        ...
        std::vector<std::vector<int>> to_forbid = arcs_to_forbid(inst);
        ...
        if (verbose > 1){
            cout << "Reducing symmetry took " << duration_reduce.count() / 1e6 << " seconds " << endl;
        }
        // Arcs to forbid to reduce symmetry
