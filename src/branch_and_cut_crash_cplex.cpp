...
//     IloInt a = getValue(IloSum(varCallback.x));
//     double b = getValue(IloSum(varCallback.x));
//     if (abs(a-b) > 1e-3) {
//         std::cerr << "xValues are not consistent" << std::endl;
//         std::cerr << "IloInt getValue(IloSum(x)) = " << a << std::endl;
//         std::cerr << "double getValue(IloSum(x)) = " << b << std::endl;
//         throw std::domain_error("xValues are not consistent");
//     }
//     nIteration++;
